 ...
 * @see ee.webmedia.xtee.model.XteeElement
 * @author margush
 */
public final class XteeSchemaCodePrinter implements SchemaCodePrinter {
	...
	public static void printType(Writer writer, SchemaType sType, XmlOptions opt)
			throws IOException {
	...
	void printInnerTypeJavaDoc(SchemaType sType) throws IOException {
		...
		emit("/**");
		if (sType.isDocumentType())
			emit(" * A document containing one " + prettyQName(name)
					+ " element.");
		else if (sType.isAttributeType())
			emit(" * A document containing one " + prettyQName(name)
					+ " attribute.");
		...
		case SchemaType.ATOMIC:
			emit(" * This is an atomic type that is a restriction of "
					+ getFullJavaName(sType) + ".");
			break;
		case SchemaType.LIST:
			emit(" * This is a list type whose items are "
					+ sType.getListItemType().getFullJavaName() + ".");
			break;
	...
	void printStaticTypeDeclaration(SchemaType sType, SchemaTypeSystem system)
			throws IOException {
		...
		emit("org.apache.xmlbeans.XmlBeans.typeSystemForClassLoader("
				+ interfaceShortName + ".class.getClassLoader(), \""
				+ system.getName() + "\")" + ".resolveHandle(\""
	...
	void printInnerType(SchemaType sType, SchemaTypeSystem system)
			throws IOException {
		emit("");
		...
		if (sType.isSimpleType()) {
			...
			for (int i = 0; i < props.length; i++) {
				...
				printPropertyGetters(prop.getName(), prop.isAttribute(),
						...
						prop.extendsJavaOption(), prop.extendsJavaArray(),
						prop.extendsJavaSingleton(), xteeTitle, i + 1L);

				if (!prop.isReadOnly()) {
					printPropertySetters(prop.getName(), prop.isAttribute(),
							prop.getJavaPropertyName(), prop.getJavaTypeCode(),
	...
	void printFactory(SchemaType sType) throws IOException {
		...
		if (sType.isAnonymousType() && !sType.isDocumentType()
				&& !sType.isAttributeType())
			fullFactory = false;
		...
		if (fullFactory) {
			...
			emit("public static "
					+ fullName
					+ " parse(java.lang.String xmlAsString, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.io.File file, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.net.URL u, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.io.InputStream is) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.io.InputStream is, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.io.Reader r) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(java.io.Reader r, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException, java.io.IOException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(javax.xml.stream.XMLStreamReader sr, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(org.w3c.dom.Node node, org.apache.xmlbeans.XmlOptions options) throws org.apache.xmlbeans.XmlException {");
			emit("  return ("
					+ fullName
			...
			emit("public static "
					+ fullName
					+ " parse(org.apache.xmlbeans.xml.stream.XMLInputStream xis) throws org.apache.xmlbeans.XmlException, org.apache.xmlbeans.xml.stream.XMLStreamException {");
			emit("  return ("
					+ fullName
	...
	void printNestedInnerTypes(SchemaType sType, SchemaTypeSystem system)
			throws IOException {
		...
		while (sType != null) {
			...
			if (!redefinition
					|| (sType.getDerivationType() != SchemaType.DT_EXTENSION && !sType
							.isSimpleType()))
				break;
			sType = sType.getBaseType();
		}
	...
	void startInterface(SchemaType sType) throws IOException {
		 ...
		 * specializedInterfaces.append(", org.apache.xmlbeans.LongValue"); if
		 * (bits <= SchemaType.SIZE_INT)
	...
	private void emitSpecializedAccessors(SchemaType sType) throws IOException {
		if (sType.getSimpleVariety() == SchemaType.ATOMIC
			...
			if (bits != parentBits
					|| sType.getBaseType().getFullJavaName() == null) {
	...
	void printStringEnumeration(SchemaType sType) throws IOException {
		...
		boolean hasBase;
		if (baseEnumType.isAnonymousType()
				&& baseEnumType.isSkippedAnonymousType()) {
	...
	String javaTypeForProperty(SchemaProperty sProp) {
		...
		case SchemaProperty.JAVA_ENUM:
			...
			return findJavaType(sType.getBaseEnumType()).replace('$', '.')
					+ ".Enum";
	...
	void printPropertyGetters(QName qName, boolean isAttr, String propertyName,
			int javaType, String type, String xtype, boolean nillable,
			boolean optional, boolean several, boolean singleton,
			String xteeTitle, Long xteeSequence) throws IOException {
		String propdesc = "\"" + qName.getLocalPart() + "\""
				+ (isAttr ? " attribute" : " element");
		...
		if (singleton) {
			...
			if (!xmltype) {
				printJavaDoc((several ? "Gets (as xml) first "
						: "Gets (as xml) the ") + propdesc);
				emit(xtype + " xget" + propertyName + "();");
			}

			if (nillable) {
				printJavaDoc((several ? "Tests for nil first "
						: "Tests for nil ") + propdesc);
				emit("boolean isNil" + propertyName + "();");
			}
		}

		if (optional) {
			printJavaDoc((several ? "True if has at least one "
					: "True if has ") + propdesc);
		...
		if (several) {
			...
			if (!xmltype) {
				if (_useJava15) {
					...
					emit("java.util.List<" + xtype + "> xget" + propertyName
							+ "List();");
				}
	...
	void printPropertySetters(QName qName, boolean isAttr, String propertyName,
			int javaType, String type, String xtype, boolean nillable,
			boolean optional, boolean several, boolean singleton)
			throws IOException {
		String safeVarName = NameUtil.nonJavaKeyword(NameUtil
				.lowerCamelCase(propertyName));
		...
		if (singleton) {
			...
			if (!xmltype) {
				printJavaDoc((several ? "Sets (as xml) first "
						: "Sets (as xml) the ") + propdesc);
				emit("void xset" + propertyName + "(" + xtype + " "
						+ safeVarName + ");");
			}
			...
			if (nillable) {
				printJavaDoc((several ? "Nils the first " : "Nils the ")
						+ propdesc);
		...
		if (optional) {
			printJavaDoc((several ? "Removes first " : "Unsets the ")
					+ propdesc);
		...
		if (several) {
			...
			emit("void set" + arrayName + "(" + type + "[] " + safeVarName
					+ "Array);");
			...
			if (!xmltype) {
				...
				emit("void xset" + arrayName + "(" + xtype + "[] "
						+ safeVarName + "Array);");

				printJavaDoc("Sets (as xml) ith " + propdesc);
				emit("void xset" + arrayName + "(int i, " + xtype + " "
						+ safeVarName + ");");
			}
			...
			if (!xmltype) {
				...
				emit("void insert" + propertyName + "(int i, " + type + " "
						+ safeVarName + ");");
	...
	void printConstructor(SchemaType sType, String shortName)
		...
		startBlock();
		emit("super(sType"
				+ (sType.getSimpleVariety() == SchemaType.NOT_SIMPLE ? ""
						: ", " + !sType.isSimpleType()) + ");");
		endBlock();
	...
	void startClass(SchemaType sType, boolean isInner) throws IOException {
		...
		if (sType.getSimpleVariety() == SchemaType.UNION) {
			...
			for (int i = 0; i < memberTypes.length; i++)
				interfaces.append(", "
						+ memberTypes[i].getFullJavaName().replace('$', '.'));
		}

		emit("public " + (isInner ? "static " : "") + "class " + shortName
				+ " extends " + baseClass + " implements "
				+ interfaces.toString());
	...
	void makeAttributeDefaultValue(String jtargetType, SchemaProperty prop,
			String identifier) throws IOException {
		String fullName = jtargetType;
		if (fullName == null)
			fullName = prop.javaBasedOnType().getFullJavaName()
					.replace('$', '.');

		emit("target = (" + fullName + ")get_default_attribute_value("
				+ identifier + ");");
	}
	...
	void printJGetArrayValue(int javaType, String type) throws IOException {
		...
		case SchemaProperty.JAVA_ENUM:
			...
			emit("    result[i] = ("
					+ type
					+ ")((org.apache.xmlbeans.SimpleValue)targetList.get(i)).getEnumValue();");
			break;
	...
	Map<QName, String[]> printStaticFields(SchemaProperty[] properties)
			throws IOException {
		...
		for (int i = 0; i < properties.length; i++) {
			...
			emit("private static final javax.xml.namespace.QName "
					+ identifiers[0] + " = ");
			indent();
			emit("new javax.xml.namespace.QName(" + uriString + ", \""
					+ name.getLocalPart() + "\");");
			outdent();
			...
			if (properties[i].acceptedNames() != null) {
				...
				if (qnames.length > 1) {
					identifiers[1] = (javaName + "$" + (i * 2 + 1))
							.toUpperCase();

					emit("private static final org.apache.xmlbeans.QNameSet "
							+ identifiers[1]
							+ " = org.apache.xmlbeans.QNameSet.forArray( new javax.xml.namespace.QName[] { ");
					indent();
					for (int j = 0; j < qnames.length; j++) {
						emit("new javax.xml.namespace.QName(\""
								+ qnames[j].getNamespaceURI() + "\", \""
	...
	void emitAddTarget(String identifier, boolean isAttr,
			boolean declareTarget, String xtype) throws IOException {
		if (isAttr)
			emit("target = (" + xtype + ")get_store().add_attribute_user("
					+ identifier + ");");
		else
			emit("target = (" + xtype + ")get_store().add_element_user("
					+ identifier + ");");
	}

	void emitPre(SchemaType sType, int opType, String identifier, boolean isAttr)
			throws IOException {
		emitPre(sType, opType, identifier, isAttr, "-1");
	}

	void emitPre(SchemaType sType, int opType, String identifier,
			boolean isAttr, String index) throws IOException {
		SchemaTypeImpl sImpl = getImpl(sType);
		if (sImpl == null)
			return;
		...
		if (ext != null) {
			if (ext.hasPreCall()) {
				emit("if ( " + ext.getStaticHandler() + ".preSet("
						+ prePostOpString(opType) + ", this, " + identifier
						+ ", " + isAttr + ", " + index + "))");
				startBlock();
			}
		}
	...
	void emitPost(SchemaType sType, int opType, String identifier,
	...
	void emitPost(SchemaType sType, int opType, String identifier,
		...
		if (ext != null) {
			...
			if (ext.hasPostCall())
				emit(ext.getStaticHandler() + ".postSet("
						+ prePostOpString(opType) + ", this, " + identifier
						+ ", " + isAttr + ", " + index + ");");
		}
	}
	...
	void emitGetTarget(String setIdentifier, String identifier, boolean isAttr,
			String index, int nullBehaviour, String xtype) throws IOException {
		assert setIdentifier != null && identifier != null;

		emit(xtype + " target = null;");

		if (isAttr)
			emit("target = (" + xtype + ")get_store().find_attribute_user("
					+ identifier + ");");
		else
			emit("target = (" + xtype + ")get_store().find_element_user("
					+ setIdentifier + ", " + index + ");");
		...
		case ADD_NEW_VALUE:
			// target already emited, no need for emitDeclareTarget(false,
			// xtype);
	...
	void printListGetter15Impl(String parentJavaName, String propdesc,
			String propertyName, String wrappedType, String xtype,
			boolean xmltype, boolean xget) throws IOException {
		String arrayName = propertyName + "Array";
		...
		printJavaDoc("Gets " + (xget ? "(as xml) " : "") + "a List of "
				+ propdesc + "s");

		emit("public java.util.List<" + wrappedType + "> " + xgetMethod
				+ listName + "()");
		startBlock();

		emit("final class " + listName + " extends java.util.AbstractList<"
				+ wrappedType + ">");
		startBlock();
	...
	void printGetterImpls(String parentJavaName, SchemaProperty prop,
			QName qName, boolean isAttr, String propertyName, int javaType,
			String type, String xtype, boolean nillable, boolean optional,
			boolean several, boolean singleton, boolean isunion,
			String identifier, String setIdentifier) throws IOException {
		String propdesc = "\"" + qName.getLocalPart() + "\""
				+ (isAttr ? " attribute" : " element");
		boolean xmltype = (javaType == SchemaProperty.XML_OBJECT);
		String jtargetType = (isunion || !xmltype) ? "org.apache.xmlbeans.SimpleValue"
				: xtype;
		...
		if (singleton) {
			...
			if (isAttr
					&& (prop.hasDefault() == SchemaProperty.CONSISTENTLY || prop
							.hasFixed() == SchemaProperty.CONSISTENTLY)) {
				emit("if (target == null)");
				startBlock();
		...
		if (optional) {
			...
			if (isAttr)
				emit("return get_store().find_attribute_user(" + identifier
						+ ") != null;");
			else
		...
		if (several) {
			...
			if (_useJava15) {
				...
				printListGetter15Impl(parentJavaName, propdesc, propertyName,
						wrappedType, xtype, xmltype, false);
			}
			...
			if (_useJava15)
				emit("java.util.List<" + xtype
						+ "> targetList = new java.util.ArrayList<" + xtype
						+ ">();");
			else
			...
			if (!xmltype) {
				if (_useJava15) {
					printListGetter15Impl(parentJavaName, propdesc,
							propertyName, xtype, xtype, xmltype, true);
				}
	...
	void printSetterImpls(QName qName, boolean isAttr, String propertyName,
			int javaType, String type, String xtype, boolean nillable,
			boolean optional, boolean several, boolean singleton,
			boolean isunion, String identifier, String setIdentifier,
		...
		if (singleton) {
			...
			printJavaDoc((several ? "Sets first " : "Sets the ") + propdesc);
			emit("public void set" + propertyName + "(" + type + " "
					+ safeVarName + ")");
			startBlock();
			emitImplementationPreamble();
			...
			if (!xmltype) {
				...
				printJavaDoc((several ? "Sets (as xml) first "
						: "Sets (as xml) the ") + propdesc);
				emit("public void xset" + propertyName + "(" + xtype + " "
						+ safeVarName + ")");
				startBlock();
				emitImplementationPreamble();
		...
		if (optional) {
			...
			startBlock();
			emitImplementationPreamble();
			emitPre(sType, PrePostExtension.OPERATION_REMOVE, identifier,
					isAttr, several ? "0" : "-1");
			if (isAttr)
				emit("get_store().remove_attribute(" + identifier + ");");
			else
				emit("get_store().remove_element(" + setIdentifier + ", 0);");
			emitPost(sType, PrePostExtension.OPERATION_REMOVE, identifier,
					isAttr, several ? "0" : "-1");
			emitImplementationPostamble();
			endBlock();
		}
		...
		if (several) {
			...
			// JSET_INDEX
			printJavaDoc("Sets array of all " + propdesc);
			emit("public void set" + arrayName + "(" + type + "[] "
					+ safeVarName + "Array)");
			startBlock();
			emitImplementationPreamble();
			emitPre(sType, PrePostExtension.OPERATION_SET, identifier, isAttr);
			...
			if (isobj) {
				...
				if (!isSubstGroup)
					emit("arraySetterHelper(" + safeVarName + "Array" + ", "
							+ identifier + ");");
				else
					emit("arraySetterHelper(" + safeVarName + "Array" + ", "
							+ identifier + ", " + setIdentifier + ");");
			...
			printJavaDoc("Sets ith " + propdesc);
			emit("public void set" + arrayName + "(int i, " + type + " "
					+ safeVarName + ")");
			startBlock();
			emitImplementationPreamble();
			emitPre(sType, PrePostExtension.OPERATION_SET, identifier, isAttr,
			...
			if (!xmltype) {
				printJavaDoc("Sets (as xml) array of all " + propdesc);
				emit("public void xset" + arrayName + "(" + xtype + "[]"
						+ safeVarName + "Array)");
				startBlock();
				emitImplementationPreamble();
				emitPre(sType, PrePostExtension.OPERATION_SET, identifier,
				...
				printJavaDoc("Sets (as xml) ith " + propdesc);
				emit("public void xset" + arrayName + "(int i, " + xtype + " "
						+ safeVarName + ")");
				startBlock();
				emitImplementationPreamble();
				emitPre(sType, PrePostExtension.OPERATION_SET, identifier,
			...
			if (!xmltype) {
				printJavaDoc("Inserts the value as the ith " + propdesc);
				emit("public void insert" + propertyName + "(int i, " + type
						+ " " + safeVarName + ")");
				startBlock();
				emitImplementationPreamble();
				emitPre(sType, PrePostExtension.OPERATION_INSERT, identifier,
				...
				if (!isSubstGroup)
					emit("(" + jtargetType
							+ ")get_store().insert_element_user(" + identifier
							+ ", i);");
				else
					// This is a subst group case
					emit("(" + jtargetType
				...
				printJavaDoc("Appends the value as the last " + propdesc);
				emit("public void add" + propertyName + "(" + type + " "
						+ safeVarName + ")");
				startBlock();
				emitImplementationPreamble();
	...
	void printInnerTypeImpl(SchemaType sType, SchemaTypeSystem system,
		...
		if (!sType.isSimpleType()) {
			...
			if (sType.getContentType() == SchemaType.SIMPLE_CONTENT) {
				// simple content types impls derive directly from "holder"
				// impls
				// in order to handle the case (for ints or string enums e.g.)
				// where
				// there is a simple type restriction. So property getters need
				// to
				// be implemented "from scratch" for each derived complex type
				// Moreover, attribute or element properties can be removed via
				// restriction,
				// but we still need to implement them because this class is
				// supposed to
				...
				while (!baseType.isSimpleType() && !baseType.isBuiltinType()) {
					SchemaProperty[] baseProperties = baseType
							.getDerivedProperties();
					for (int i = 0; i < baseProperties.length; i++)
						if (!(baseProperties[i].isAttribute() && sType
								.getAttributeProperty(baseProperties[i]
										.getName()) != null)) {
				...
				// complex content type implementations derive from base type
				// impls
			...
			for (int i = 0; i < properties.length; i++) {
				...
				if (!prop.isReadOnly()) {
					printSetterImpls(name, prop.isAttribute(),
							prop.getJavaPropertyName(), prop.getJavaTypeCode(),
							javaTypeForProperty(prop), xmlType,
							prop.hasNillable() != SchemaProperty.NEVER,
							prop.extendsJavaOption(), prop.extendsJavaArray(),
							...
							xmlTypeForPropertyIsUnion(prop),
							getIdentifier(qNameMap, name),
							getSetIdentifier(qNameMap, name), sType);
				}
			}
		}
	...
	void printInterfaceMethodImpl(String handler,
			InterfaceExtension.MethodSignature method) throws IOException {
		StringBuffer impl = new StringBuffer(60);

		if (!method.getReturnType().equals("void"))
			impl.append("return ");

		impl.append(handler).append(".").append(method.getName())
				.append("(this");
	...
	void printNestedTypeImpls(SchemaType sType, SchemaTypeSystem system)
			throws IOException {
		boolean redefinition = sType.getName() != null
				&& sType.getName().equals(sType.getBaseType().getName());
		while (sType != null) {
	...
	void printXteeElement(String xteeTitle, Long xteeSequence) throws IOException{
		if (StringUtils.isNotEmpty(xteeTitle)) {
			emit("@ee.webmedia.xtee.model.XteeElement(title=\"" + xteeTitle + "\", sequence="+ xteeSequence + ")");
		}
	}
